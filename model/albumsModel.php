...
class albumsModel extends Model
{
  ...
  function addAlbum($name, $year, $artist, $genre, $info){
    ...
    $sentence = $this->db->prepare("INSERT INTO `album`(`nombre`, `anio`, `artista`, `genero`, `id_genero`, `descripcion`) VALUES (?,?,?,?,?,?)");
    return $sentence->execute([$name, $year, $artist, $genre, $id_genre['id_genero'], $info]);
  }
  ...
  function updateAlbum($name, $year, $artist, $genre, $id_album, $info){
    $id_genre =  $this->getGenreID($genre);
    $sentence = $this->db->prepare("UPDATE `album` SET `nombre` = ?, `anio` = ?, `artista` = ?, `genero` = ?, `id_genero` = ?, `descripcion` = ? WHERE `id_album` = ?");
    return $sentence->execute([$name, $year, $artist, $genre, $id_genre['id_genero'],  $info, $id_album]);
  }
